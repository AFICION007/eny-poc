...
const INITIAL_DOCUMENTS = [
  ...
  },
  {
    key: "3",
    documentName:
      "4QFY24 review_Swiggy (Rating_Revised to ADD, TP_ Rs.210).pdf",
    format: "PDF",
...
const Documents = () => {
    ...
    modal.confirm({
      ...
      onOk() {
        const remaining = documents.filter(
          (doc) => !selectedRowKeys.includes(String(doc.key))
        );
  ...
  const handleSelectAll = (e) => {
    ...
    const allFileTypes = checked
      ? FILE_TYPE_OPTIONS.map((option) => option.value)
      : [];
  ...
  const handleDownload = () => {
    selectedRowKeys.forEach((key) => {
      ...
      if (doc) {
        const blob = new Blob([`You downloaded ${doc.documentName}`], {
          type: "text/plain",
        });
    ...
    <div className={styles.documents_container}>
      ...
      <div className={styles.document_section}>
        <div className={styles.document_grid}>
          {DOCUMENTS.filter((doc) => doc.format === selectedFileTypes).map(
            (doc) => (
